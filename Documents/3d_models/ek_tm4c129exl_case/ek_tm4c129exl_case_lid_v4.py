...
def build_model() -> tuple[list[Box], list[Box], tuple[float, float, float]]:
    ...
    for x0_mm, y0_mm, x1_mm, y1_mm in HEADER_BBOXES_MM:
        ...
        if y0_mm < (BOARD_WIDTH_MM / 2.0):
            negatives.append(Box(board_to_lid_x(x0_mm - HEADER_SIDE_MARGIN_MM), board_to_lid_x(x1_mm + HEADER_SIDE_MARGIN_MM), -1.0, board_to_lid_y(y1_mm + HEADER_SIDE_MARGIN_MM), PORT_CUTOUT_Z0_MM, PORT_CUTOUT_Z1_MM))
        else:
            negatives.append(Box(board_to_lid_x(x0_mm - HEADER_SIDE_MARGIN_MM), board_to_lid_x(x1_mm + HEADER_SIDE_MARGIN_MM), board_to_lid_y(y0_mm - HEADER_SIDE_MARGIN_MM), OUTER_WIDTH_MM + 1.0, PORT_CUTOUT_Z0_MM, PORT_CUTOUT_Z1_MM))
...
def write_stl(output_path: Path) -> None:
    ...
    lines = ["solid ek_tm4c129exl_case_lid_v4"]
    lines.extend(facets)
    lines.append("endsolid ek_tm4c129exl_case_lid_v4")
