...
def build_model() -> tuple[list[Box], list[Box], tuple[float, float, float]]:
    ...
    for x0_mm, y0_mm, x1_mm, y1_mm in LEFT_PORT_BBOXES_MM:
        negatives.append(
            Box(
                -1.0,
                board_to_case_x(x1_mm + PORT_MARGIN_MM),
                board_to_case_y(y0_mm - PORT_MARGIN_MM),
                board_to_case_y(y1_mm + PORT_MARGIN_MM),
                PORT_CUTOUT_Z0_MM,
                CASE_HEIGHT_MM + 1.0,
            )
        )
    ...
    for x0_mm, y0_mm, x1_mm, y1_mm in RIGHT_PORT_BBOXES_MM:
        negatives.append(
            Box(
                board_to_case_x(x0_mm - PORT_MARGIN_MM),
                OUTER_LENGTH_MM + 1.0,
                board_to_case_y(y0_mm - PORT_MARGIN_MM),
                board_to_case_y(y1_mm + PORT_MARGIN_MM),
                PORT_CUTOUT_Z0_MM,
    ...
    for x0_mm, y0_mm, x1_mm, y1_mm in BOTTOM_HEADER_BBOXES_MM:
        negatives.append(
            Box(
                board_to_case_x(x0_mm - BOTTOM_HEADER_MARGIN_MM),
                board_to_case_x(x1_mm + BOTTOM_HEADER_MARGIN_MM),
                board_to_case_y(y0_mm - BOTTOM_HEADER_MARGIN_MM),
                board_to_case_y(y1_mm + BOTTOM_HEADER_MARGIN_MM),
                0.0,
                BASE_THICKNESS_MM + 0.1,
            )
        )

    return positives, negatives, (OUTER_LENGTH_MM, OUTER_WIDTH_MM, CASE_HEIGHT_MM)
...
def write_stl(output_path: Path) -> None:
    ...
    lines = ["solid ek_tm4c129exl_case_tray_v2"]
    lines.extend(facets)
    lines.append("endsolid ek_tm4c129exl_case_tray_v2")
