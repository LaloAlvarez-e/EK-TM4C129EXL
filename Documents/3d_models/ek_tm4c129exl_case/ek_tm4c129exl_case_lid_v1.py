...
PORT_MARGIN_MM = 1.0
PORT_CUTOUT_Z0_MM = 0.5
PORT_CUTOUT_Z1_MM = LID_SKIRT_DEPTH_MM + 1.0
...
def build_model() -> tuple[list[Box], list[Box], tuple[float, float, float]]:
    ...
    negatives: list[Box] = [
        Box(
            ...
            OUTER_WIDTH_MM - LID_WALL_THICKNESS_MM,
            0.0,
            LID_HEIGHT_MM - LID_ROOF_THICKNESS_MM,
        )
    ]
    ...
    roof_z0 = LID_HEIGHT_MM - LID_ROOF_THICKNESS_MM - 0.1
...
def write_stl(output_path: Path) -> None:
    ...
    lines = ["solid ek_tm4c129exl_case_lid_v1"]
    lines.extend(facets)
    lines.append("endsolid ek_tm4c129exl_case_lid_v1")
